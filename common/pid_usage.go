...
// ProcessInfo will record cpu and memory and start data
...
type fn func(int) (*ProcessInfo, error)
...
func wrapper(statType string) func(pid int) (*ProcessInfo, error) {
...
func stat(pid int, statType string) (*ProcessInfo, error) {
	sysInfo := &ProcessInfo{}
	...
	if statType == "ps" {
		...
		linuxBootTimeStamp := time.Now().Unix() - int64(math.Floor(uptime))
		sysInfo.StartTime = time.Unix(linuxBootTimeStamp+int64(math.Floor(stat.start)), 0)
	}
	return sysInfo, nil

}

// GetStat will return process current use system CPU and memory data
func GetStat(pid int) (*ProcessInfo, error) {
